...
ggplot(dd,aes(x=x,y=y))+geom_point(col='#a5f0a1')+facet_wrap(~dataset,nrow=3)
comment('datasaurus_dozen안에 들어 있는 dataset별로 여러 개의 정보를 바탕으로 그림을 그려줌')

ggplot(dd,aes(x=x,y=y,colour=dataset))+geom_point()+facet_wrap(~dataset,nrow=3)
comment('나오는 그래프의 색을 다 다르게 함')
  
## 출처: https://cran.r-project.org/web/packages/datasauRus/vignettes/Datasaurus.html
ggplot(dd, aes(x = x, y = y, colour = dataset))+geom_point()+theme_void()+theme(legend.position = "none")+facet_wrap(~dataset, ncol = 3)
...
ggplot(gapminder::gapminder) +
 ...
 labs(x = "소득", y = "기대수명", title = "gapminder따라하기", 
 subtitle = "good", caption = "hans copycats") +
 theme_gray() +
